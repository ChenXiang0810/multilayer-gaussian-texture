...
def render_view(camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, 
    ...
    if override_color is None:
        if pipe.compute_SHs_python:
            if not pc.use_brdf:
                ...
                rays_d = pc.get_xyz - camera.camera_center.repeat(pc.get_xyz.shape[0], 1)
                ...
                if pc.stage == 3:  # For encoder-decoder stage3
                    latent = pc.get_latent  # eval
                    # latent = torch.sigmoid(pc.get_latent)  # train
                    brdf = pc.get_decoder(latent)
                elif pc.stage == 2:  # For encoder-decoder stage2
                    trained_normal = (pc.get_normal.detach() + 1) * 0.5
                    net_input = torch.cat([pc.get_shs.reshape(-1, 48).detach(), trained_normal], dim=-1)
                    brdf = pc.get_decoder(pc.get_encoder(pc.get_encoding(net_input)))
                    # brdf = pc.get_decoder(torch.sigmoid(pc.get_encoder(pc.get_encoding(net_input))))
                ...
                # reflection formula: w_i = 2 * |w_o * n| * n - w_o
                rays_d = pc.get_xyz - camera.camera_center.repeat(means3D.shape[0], 1)
                rays_d = rays_d / rays_d.norm(dim=1, keepdim=True)
                cos_theta = -(rays_d * normal).sum(dim=-1, keepdim=True)
                reflect_d = 2 * cos_theta * normal + rays_d  # N_rays*N_samples, 3
                reflect_d = reflect_d / reflect_d.norm(dim=1, keepdim=True)
    ...
    if pc.use_brdf:
        if not pipe.compute_SHs_python_defer:
            ...
            features = torch.cat([features, pc.get_latent], dim=-1)
    ...
    if not pc.use_brdf:
        ...
        rendered_normal, rendered_depth, rendered_depth2, rendered_latent = torch.split(rendered_feature, [3, 1, 1, 16], dim=0)
    ...
    if pc.use_brdf:
        if not pipe.compute_SHs_python_defer:
            ...
            results["latent"] = rendered_latent
            H, W = rendered_latent.shape[1:]
            defer_latent = rendered_latent.permute(1, 2, 0).reshape([H*W, -1])

            envSHs = pc.get_light[None]
            brdf = pc.get_decoder(defer_latent)
            albedo = torch.sigmoid(brdf[..., 0:3])
            specular = torch.sigmoid(brdf[..., 3:4])
            s = torch.nn.functional.softplus(brdf[..., 4:5]) + 1
            ...
            # flag_sign = torch.where(normal[..., 2:3] > 0, torch.ones_like(normal[..., 2:3]), -torch.ones_like(normal[..., 2:3])).detach()
            # normal = normal * flag_sign
            results["normal"] = normal.reshape([H, W, -1]).permute(2, 0, 1)
            results["albedo"] = albedo.reshape([H, W, -1]).permute(2, 0, 1)
            results["specular"] = specular.reshape([H, W, -1]).permute(2, 0, 1)
            ...
            # reflection formula: w_i = 2 * |w_o * n| * n - w_o
            rays_d = camera.get_world_directions()  # may be wrong!!!
            ...
            specular_rgb = render_irrandiance_sh_sum(sh_coeff, reflect_d)
            specular_rgb = torch.clamp_min(specular_rgb, 0.0)
            specular = specular * specular_rgb

            radiance = torch.clamp_min(diffuse + specular, 0.0)  # * mask_back
