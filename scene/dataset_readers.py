...
def nerf_matrix_to_ngp(pose, scale=0.8):  # scale=0.8 scale=0.33
...
def readCamerasFromNerfTexture(path, transformsfile, extension=".png", debug=False, white_background=False, downscale=1):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        if 'h' in contents and 'w' in contents:
            H = int(contents['h']) // downscale
            W = int(contents['w']) // downscale
        else:
            H = W = 800

        # load intrinsics
        if 'fl_x' in contents or 'fl_y' in contents:
            fl_x = (contents['fl_x'] if 'fl_x' in contents else contents['fl_y']) / downscale
            fl_y = (contents['fl_y'] if 'fl_y' in contents else contents['fl_x']) / downscale
        elif 'camera_angle_x' in contents or 'camera_angle_y' in contents:
            # blender, assert in radians. already downscaled since we use H/W
            fl_x = W / (2 * np.tan(fovx / 2)) if 'camera_angle_x' in contents else None
            fl_y = H / (2 * np.tan(fovy / 2)) if 'camera_angle_y' in contents else None
            if fl_x is None: fl_x = fl_y
            if fl_y is None: fl_y = fl_x
        else:
            raise RuntimeError('Failed to load focal length, please check the transforms.json!')

        cx = (contents['cx'] / downscale) if 'cx' in contents else (H / 2)
        cy = (contents['cy'] / downscale) if 'cy' in contents else (W / 2)
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            # c2w[:3, 1:3] *= -1
            c2w = nerf_matrix_to_ngp(c2w)  # for real dataset training
            # c2w = nerf_matrix_to_ngp(c2w, scale=1.0)
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, fx=fl_x, fy=fl_y,
                                        cx=cx, cy=cy, image=image, image_mask=image_mask, image_path=image_path,
                                        image_name=image_name, width=W, height=H))
...
def getNGPppNorm(cam_info):
    ...
    cam_poses, _ = normalize_cps(cam_poses, scale=1.2)  # for real dataset training

    for cam in cam_info:
        w2c = np.linalg.inv(cam_poses[cam_id])
        R = np.transpose(w2c[:3, :3])  # R is stored transposed due to 'glm' in CUDA code
        T = w2c[:3, 3]
        cam_info[cam_id] = cam._replace(R=R, T=T)

        cam_centers.append(cam_poses[cam_id, :3, 3:4])
        cam_id = cam_id + 1
...
def readNerfTextureInfo(path, images, eval, llffhold=8, debug=False):
    # load nerf-compatible format data.
    cam_infos_unsorted = readCamerasFromNerfTexture(path, "transforms.json")
    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: x.image_name)

    train_cam_infos = cam_infos
    test_cam_infos = []

    nerf_normalization = getNGPppNorm(train_cam_infos)  # for real dataset training
    ...
    if not os.path.exists(ply_path):
        ...
        xyz = np.concatenate([uv, z_val], axis=-1)
...
sceneLoadTypeCallbacks = {
    ...
    "NerfTexture": readNerfTextureInfo,
