...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.brdf_activation = torch.sigmoid
        self.glossiness_activation = torch.nn.Softplus()

    def __init__(self, sh_degree: int, render_type='render', use_brdf=False, use_neural=False, stage=1):
        self.render_type = render_type
        self.use_pbr = render_type in ['neilf']
        self.use_brdf = use_brdf
        ...
        if self.use_brdf:
            if self.use_neural:
                ...
                self._glossiness = torch.empty(0)
        # self._light = None
        self._light_dc = None
        self._light_rest = None
        self.decoder = None
        self.encoder = None
        self.encoding = None
    ...
    def get_light(self):
        # return self._light
        return torch.cat((self._light_dc, self._light_rest), dim=-1)

    @property
    def get_albedo(self):
        return self.brdf_activation(self._albedo)

    @property
    def get_specular(self):
        return self.brdf_activation(self._specular)

    @property
    def get_glossiness(self):
        return self.glossiness_activation(self._glossiness) + 1
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        if self.use_brdf:
            self.create_light_sh(3, 1)  # Init light
            if self.use_neural:
                latents = torch.ones((fused_point_cloud.shape[0], 16), device="cuda")
                self._latent = nn.Parameter(latents.requires_grad_(True))
                # self.create_decoder(64, 2)  # Init decoder
                self.create_decoder(16, 2)  # Init decoder
                # self.create_encoder(128, 4)  # Init encoder
                self.create_encoder(64, 2)  # Init encoder
            else:
                albedos = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
                speculars = torch.zeros((fused_point_cloud.shape[0], 1), device="cuda")
                glossinesses = torch.zeros((fused_point_cloud.shape[0], 1), device="cuda")
                self._albedo = nn.Parameter(albedos.requires_grad_(True))
                self._specular = nn.Parameter(speculars.requires_grad_(True))
                self._glossiness = nn.Parameter(glossinesses.requires_grad_(True))
    ...
    def training_setup(self, training_args: OptimizationParams):
        ...
        l = [
            ...
            # {'params': [self._shs_rest], 'lr': training_args.sh_lr, "name": "f_rest"}
        ]
        ...
        if self.use_brdf:
            l.extend([
                # {'params': [self._light], 'lr': training_args.light_lr_init, "name": "light"}
                {'params': [self._light_dc], 'lr': training_args.light_lr_dc, "name": "light_dc"},
                {'params': [self._light_rest], 'lr': training_args.light_lr_rest, "name": "light_rest"},
            ])
            if self.use_neural:
                l.extend([
                    {'params': [self._latent], 'lr': training_args.feature_lr, "name": "latent"},  # *1 For decoder-only
                    {'params': [param for param in self.decoder.parameters()], 'lr': training_args.feature_lr * 0.1, "name": "decoder"},  # * 0.1  *0 For encoder-decoder stage3
                    {'params': [param for param in self.encoder.parameters()], 'lr': training_args.feature_lr, "name": "encoder"}
                ])
            else:
                l.extend([
                    {'params': [self._albedo], 'lr': training_args.feature_lr, "name": "albedo"},
                    {'params': [self._specular], 'lr': training_args.feature_lr, "name": "specular"},
                    {'params': [self._glossiness], 'lr': training_args.feature_lr, "name": "glossiness"}
                ])
        ...
        self.light_scheduler_args = get_expon_lr_func(training_args.light_lr_init, training_args.light_lr_final,
                                                        lr_delay_steps=training_args.light_lr_delay_steps,
                                                        lr_delay_mult=training_args.light_lr_delay_mult,
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
                return lr
            if param_group["name"] == "light":
                lr = self.light_scheduler_args(iteration)
                param_group['lr'] = lr
                return lr
    ...
    def construct_list_of_attributes(self):
        ...
        if self.use_brdf:
            if self.use_neural:
                for i in range(self._latent.shape[1]):
    ...
    def save_ply(self, path):
        ...
        if self.use_brdf:
            if self.use_neural:
                if self.stage == 2:
                    attributes_list.extend([
                        # self.encoder(torch.cat([self._shs_dc, self._shs_rest], dim=1).reshape(-1, 48)).detach().cpu().numpy(),  # For encoder-decoder stage2
                        self.encoder(self.encoding(torch.cat([torch.cat([self._shs_dc, self._shs_rest], dim=1).reshape(-1, 48), (self.normal_activation(self._normal) + 1) * 0.5], dim=-1))).detach().cpu().numpy(),  # For encoder-decoder stage2
                    ])
                elif self.stage == 3:
                    attributes_list.extend([
                        self._latent.detach().cpu().numpy(),  # For encoder-decoder stage3 or decoder-only
                    ])
            else:
                attributes_list.extend([
                    self._albedo.detach().cpu().numpy(),
                    self._specular.detach().cpu().numpy(),
                    self._glossiness.detach().cpu().numpy(),
                ])
    ...
    def load_ply(self, path):
        ...
        if self.use_brdf:
            if self.use_neural:
                latent_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("latent_")]
                latent_names = sorted(latent_names, key = lambda x: int(x.split('_')[-1]))
                # latents = np.ones((xyz.shape[0], len(latent_names)))
                # latents = np.ones((xyz.shape[0], 8))  # 16  4 for encoder-decoder
                latents = np.random.random((xyz.shape[0], 8))  # 16  4 for encoder-decoder
                # latents = np.concatenate([np.mean(shs_dc, axis=1), np.mean(shs_extra, axis=1)], axis=-1)
                for idx, attr_name in enumerate(latent_names):
                    latents[:, idx] = np.asarray(plydata.elements[0][attr_name])
                self._latent = nn.Parameter(torch.tensor(latents, dtype=torch.float, device="cuda").requires_grad_(True))
            else:
                ...
                # albedos = np.zeros((xyz.shape[0], len(albedo_names)))
                albedos = np.zeros((xyz.shape[0], 3))
                flag_load = False
                for idx, attr_name in enumerate(albedo_names):
                    albedos[:, idx] = np.asarray(plydata.elements[0][attr_name])
                    flag_load = True
                speculars = np.zeros((xyz.shape[0], 1))
                glossinesses = np.zeros((xyz.shape[0], 1))
                if flag_load:
                    speculars = np.asarray(plydata.elements[0]["specular"])[..., np.newaxis]
                    glossinesses = np.asarray(plydata.elements[0]["glossiness"])[..., np.newaxis]
                ...
                self._glossiness = nn.Parameter(torch.tensor(glossinesses, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def create_light_sh(self, light_order, light_dim, intensity=1):
        init_light = torch.zeros([light_dim, (light_order + 1) * (light_order + 1)])
        # initialize lighting with a white ambient light
        init_light[:, 0] = 3 * intensity
        # self._light = torch.nn.Parameter(init_light.cuda(), requires_grad=True)
        self._light_dc = torch.nn.Parameter(init_light[:, 0:1].cuda(), requires_grad=True)
        self._light_rest = torch.nn.Parameter(init_light[:, 1:].cuda(), requires_grad=True)

    def set_light_sh(self, path):
        self.use_brdf = True
        init_light = torch.Tensor(np.load(path)).cuda()  #  * 0.6
        # self._light = torch.nn.Parameter(init_light, requires_grad=True)
        self._light_dc = torch.nn.Parameter(init_light[:, 0:1].cuda(), requires_grad=True)
        self._light_rest = torch.nn.Parameter(init_light[:, 1:].cuda(), requires_grad=True)
    ...
    def save_light(self, path):
        if self.use_brdf:
            mkdir_p(os.path.dirname(path))
            np.save(path, self.get_light.detach().cpu().numpy())

    def save_decoder_params(self, path):
        if self.use_neural:
            torch.save(self.decoder.state_dict(), path)

    def set_decoder(self, path):
        self.use_neural = True
        self.create_decoder()
        self.decoder.load_state_dict(torch.load(path))

    def create_decoder(self, neurons=32, layers=2):  # 64 for decoder-only  16 for encoder-decoder
        self.decoder = tcnn.Network(
            # n_input_dims=self._latent.data.shape[-1],
            n_input_dims=8,  # 48 for decoder-only  4 for encoder-decoder
            n_output_dims=8,  # albedo[3], specular[1], glossiness[1], normal[3]
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
    ...
    def save_encoder_params(self, path):
        if self.use_neural:
            torch.save(self.encoder.state_dict(), path)

    def set_encoder(self, path):
        self.use_neural = True
        self.create_encoder()
        self.encoder.load_state_dict(torch.load(path))

    def create_encoder(self, neurons=128, layers=2):
        self.encoder = tcnn.Network(
            n_input_dims=64,  # 16 * 3 SH 48
            n_output_dims=8,  # 16 Latent 4 for encoder-decoder
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
    ...
    def get_smallest_axis(self, return_idx=False):
        rot_m = build_rotation(self._rotation)
        min_axis_idx = torch.flip(self.get_scaling, dims=[-1]).min(dim=-1)[1][..., None, None].expand(-1, 3, -1)  # default z
        min_axis = rot_m.gather(2, 2 - min_axis_idx)  # 0 to z
        if return_idx:
            return min_axis.squeeze(dim=2), min_axis_idx[..., 0, 0]
        return min_axis.squeeze(dim=2)

    def get_geometry_normal(self):
        normal_global = self.get_smallest_axis()
        direction_ref = torch.zeros_like(self._xyz)
        direction_ref[..., 2] = 1
        neg_mask = (normal_global * direction_ref).sum(-1) < 0.0
        normal_global[neg_mask] = -normal_global[neg_mask]
        return torch.nn.functional.normalize(normal_global, dim=-1)
